# anthropic-claude-code-print_20260902

source: <https://github.com/Continuum-AI-Corp/OrcaReplay/blob/main/prompt/CLAUDECODE/claude-fable-5-1-print-system-prompt.md>

reproduce: `node capture/capture.mjs claude --print --model claude-fable-5-1` — captured off the wire with [OrcaReplay](https://github.com/Continuum-AI-Corp/OrcaReplay) (Apache-2.0), a local proxy in front of the model API recording the request the harness actually sent. Claude Code CLI v2.1.258.18d in `-p` (print) mode, model `claude-fable-5-1`, billing header `cc_entrypoint=sdk-cli`; 20,938 characters and 29 tool definitions as sent. This is a different prompt from the interactive one, not the same text reflowed: different identity line, no `! <command>` hint, no Scratchpad Directory section, no `gitStatus` block, and six fewer tools. Machine-identifying strings are replaced with `{PLACEHOLDER}` tokens; nothing else is reworded or reordered.
