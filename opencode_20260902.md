...
source: <https://github.com/Continuum-AI-Corp/OrcaReplay/blob/main/prompt/OPENCODE/big-pickle-system-prompt.md>

reproduce: `node capture/capture.mjs opencode --model big-pickle` — captured off the wire with [OrcaReplay](https://github.com/Continuum-AI-Corp/OrcaReplay) (Apache-2.0), a local proxy in front of the model API recording the request the harness actually sent. opencode in `run` mode, model `big-pickle`; 9,621 characters and 11 tool definitions as sent. opencode picks its prompt per model - this is the template five of the seven models measured receive, within a few dozen characters of each other. Machine-identifying strings are replaced with `{PLACEHOLDER}` tokens; nothing else is reworded or reordered.
