# openai-codex-cli_20260902

source: <https://github.com/Continuum-AI-Corp/OrcaReplay/blob/main/prompt/CODEX/gpt-5-6-sol-system-prompt.md>

reproduce: `node capture/capture.mjs codex --model gpt-5.6-sol` — captured off the wire with [OrcaReplay](https://github.com/Continuum-AI-Corp/OrcaReplay) (Apache-2.0), a local proxy in front of the model API recording the request the harness actually sent. OpenAI Codex CLI in `exec` mode, model `gpt-5.6-sol`, Responses API dialect; 23,377 characters and 9 tool definitions as sent. Machine-identifying strings are replaced with `{PLACEHOLDER}` tokens; nothing else is reworded or reordered.
